...
import com.mdvns.mdvn.comment.sapi.repository.ReplyDetailRepository;
...
public class CommentServiceImpl implements CommentService {
    ...
    @Autowired
    private ReplyDetailRepository replyDetailRepository;
    ...
    public CreateCommentInfoResponse createCommentInfo(CreateCommentInfoRequest request) {
        ...
        String replyId = request.getCommentId();
        ...
        if (passiveAtList != null && passiveAtList.size() > 0) {
            ...
            for (int i = 0; i < passiveAtList.size(); i++) {
                String passiveAt = passiveAtList.get(i);
                ReplyDetail replyDetail = new ReplyDetail();
                replyDetail.setCreatorId(creatorId);
                replyDetail.setCommentId(comment.getCommentId());
                replyDetail.setIsDeleted(0);
                replyDetail.setPassiveAt(passiveAt);
                replyDetail.setCreateTime(currentTime);
                if (!StringUtils.isEmpty(replyId)) {
                    replyDetail.setReplyId(replyId);
                    //判断是相互第几次@，并且算出间隔的时间
                    ReplyDetail judgeReplyDetail = this.replyDetailRepository.findByReplyIdAndCreatorIdAndPassiveAt(replyId, passiveAt, creatorId);
                    //间隔时间
                    if (judgeReplyDetail != null) {
                        long intervalTime = currentTime.getTime() - judgeReplyDetail.getCreateTime().getTime();
                        replyDetail.setIntervalTime(intervalTime);
                    }
                }
                replyDetails.add(replyDetail);
            }
            replyDetails = this.replyDetailRepository.save(replyDetails);
            createCommentInfoResponse.setReplyDetails(replyDetails);
        }
    ...
    public List<CommentDetail> rtrvCommentInfos(RtrvCommentInfosRequest request) {
        ...
        for (int i = 0; i < comments.size() ; i++) {
            ...
            List<ReplyDetail> replyDetails = this.replyDetailRepository.findByCommentIdAndIsDeleted(comment.getCommentId(),0);
