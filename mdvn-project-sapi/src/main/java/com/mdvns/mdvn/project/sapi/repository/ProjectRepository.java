...
public interface ProjectRepository extends JpaRepository<Project, Integer>{
    ...
    //查询所有project(先按状态排序，再按时间排序，最后按优先级排序)
    @Query(value="SELECT t.* ,(CASE WHEN STATUS=\"new\" THEN 1 WHEN STATUS=\"going\" THEN 2 WHEN STATUS=\"reopen\" THEN 3 WHEN STATUS=\"finished\" THEN 4 WHEN STATUS=\"pause\" THEN 5 ELSE 6 END) state FROM project t ORDER BY state ,create_time DESC , priority DESC LIMIT ?1,?2", nativeQuery = true)
    List<Project> rtrvProjInfoList(Integer m,Integer n);
